...
abundant numbers.
*)
type Solution023Sandbox() =
    member this.Solution023 =
        let factorize = Factorize()
        let abundants =
            factorize.GetAbundants
            |> Seq.takeWhile (fun elem -> elem < 28123L)
            |> Seq.cache
        let sumOfComposites =
            abundants
            |> Seq.collect (fun x -> abundants |> Seq.map (fun y -> x + y) |> Seq.takeWhile (fun elem -> elem < 28123L))
            |> Seq.distinctBy id
            |> Seq.sum
        let sumOfAll =
            [1L..28122L]
            |> List.sum
        sumOfAll - sumOfComposites
